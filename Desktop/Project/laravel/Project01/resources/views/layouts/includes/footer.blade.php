
<footer class="container-fluid show-footer ms-0 p-4">
    <div class="row">
        <div class="col-lg-1 footer-logo-mit d-flex align-items-strat justify-content-center">
            <img src="{{ asset('images/Massachusetts_Institute_of_Technology-Logo.wine.png') }}" alt=""
                width="100%" height="50%">
        </div>
        <div class="col-lg-11 footer-menu-mit">
            <div class="row">
                <div class="col-lg-6 d-flex flex-column align-items-start justify-content-start">
                    <h6>Massachusetts Institute of Technology</h6>
                    <p>77 Massachusetts Avenue, Cambridge, MA, USA</p>
                    <ul class="d-flex gap-4">
                        <li><a href="">Visit</a></li>
                        <li><a href="">Map</a></li>
                        <li><a href="">Events</a></li>
                        <li><a href="">People</a></li>
                        <li><a href="">Careers</a></li>
                        <li><a href="">Contact</a></li>
                    </ul>
                    <ul class="d-flex gap-4">
                        <li><a href="">Privacy</a></li>
                        <li><a href="">Accessibility</a></li>
                        <li><a href="">Social Media Hub</a></li>
                        <li><a href=""><i class="fa-brands fa-x-twitter"></i></a></li>
                        <li><a href=""><i class="fa-brands fa-facebook-f"></i></a></li>
                        <li><a href=""><i class="fa-brands fa-youtube"></i></a></li>
                        <li><a href=""><i class="fa-brands fa-instagram"></i></a></li>
                    </ul>
                </div>
                <div class="col-lg-6">
                    <div class="row">
                        <div class="col-lg-8 d-flex">
                            <ul style="border-left: 1px solid black;">
                                <li><a href="">Education</a></li>
                                <li><a href="">Admissions + Aid</a></li>
                                <li><a href="">About MIT</a></li>
                            </ul>
                            <ul class="ms-2 " style="border-left: 1px solid black;">
                                <li><a href="">Research</a></li>
                                <li><a href="">Campus Life</a></li>
                                <li><a href="">Alumni</a></li>
                            </ul>
                            <ul class="ms-2" style="border-left: 1px solid black">
                                <li><a href="">Innovation</a></li>
                                <li><a href="">News</a></li>
                            </ul>

                        </div>
                        <div class="col-lg-4">
                            <div class="footer-box d-flex  justify-content-center">
                                <p class="footer-paragrab ">Join us <br>
                                    in building a <br>
                                    better world.</p>
                                <i class="fa-solid fa-arrow-up p-1 "></i>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</footer>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet"
    integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"
    integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous">
</script>

@stack('script')
</body>

</html>
